...
Alternatively, you can use a string for the default language and provide translations using the [`translations`](#translations) property:
...
The `text` value can be a string, or for multilingual sites, an object with values for each different locale.
When using the object form, the keys must be BCP-47 tags (e.g. `en`, `fr`, or `zh-CN`):
...
Alternatively, you can use a string for the default language and provide translations using the `badge.translations` property:
...
This property provides an alternative way to specify translations for multilingual sites.
Use this when you want to specify the default language text in the [`label`](#label) property and provide translations for other locales.
The keys must be BCP-47 tags (e.g. `es`, `fr`, or `zh-CN`):
...
starlightSidebarTopics([
  {
    ...
    items: [{ label: 'Introduction', slug: 'guides/intro' }],
  },
])
```

This follows the same pattern as regular Starlight sidebar items, making it easier to maintain consistent translation approaches across your documentation.

:::tip
Both the object notation (`label: { en: '...', fr: '...' }`) and the `translations` property (`label: '...', translations: { fr: '...' }`) are supported. Choose the approach that best fits your project's needs.
:::
